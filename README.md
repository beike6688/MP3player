...
- 直接双击 `publish-single\Mp3Player_v1.2.0.exe` 即可运行，无需安装 .NET（运行时已打包进单文件，文件名中的版本号随每次发布更新）。
...
- 播放列表右键菜单：播放 / 从列表移除 / 打开文件位置
- 切歌、启动播放器、从托盘恢复窗口时，播放列表自动滚动到当前播放歌曲
...
- 星空背景：100 颗星星闪烁发光
- 深蓝黑渐变窗口背景 + 左右紫/蓝光晕 + 歌曲封面低透明度铺满背景
...
- 5 段均衡器（低音 / 中低 / 中音 / 中高 / 高音，-12dB ~ +12dB），自绘渐变发光滑块，支持一键重置
- 记住上次播放位置，重新打开自动续播
- 记住音量、播放模式、窗口位置与大小、播放列表

### 扩展功能

- 系统托盘：关闭窗口最小化到托盘，单击图标显示窗口/置顶，右键菜单含播放控制、关于（显示软件版本）
...
│  ├─ Mp3Player_v1.2.0.exe    # 单文件自包含可执行程序（文件名带版本号）
...
├─ Assets\app.ico             # 应用图标
├─ Assets\screenshot.png      # README 界面截图
...
- 版本号格式为 `主版本.次版本.修订版本`（当前 1.2.0，修订版本暂固定为 0），维护在 `Mp3Player.csproj` 的 `<Version>` 中
- 小改动（修复 bug、界面微调）：次版本号 +1，如 `1.2.0 → 1.3.0`
- 大改动（新功能、界面重构、重大变更）：主版本号 +1，如 `1.3.0 → 2.0.0`
- 发布后把 exe 重命名为 `Mp3Player_v<版本号>.exe`，并同步更新桌面快捷方式

发布注意事项：

- 发布前先关闭正在运行的播放器进程，否则 exe 被占用会发布失败
