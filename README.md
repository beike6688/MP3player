...
- 直接双击 `publish-single\Mp3Player_v1.1.0.exe` 即可运行，无需安装 .NET（运行时已打包进单文件，文件名中的版本号随每次发布更新）。
...
- 系统托盘：关闭窗口最小化到托盘，单击图标显示窗口/置顶，托盘右键菜单
...
│  ├─ Mp3Player_v1.1.0.exe    # 单文件自包含可执行程序（文件名带版本号）
...
- 版本号格式为 `主版本.次版本.修订版本`（当前 1.1.0，修订版本暂固定为 0），维护在 `Mp3Player.csproj` 的 `<Version>` 中
- 小改动（修复 bug、界面微调）：次版本号 +1，如 `1.1.0 → 1.2.0`
- 大改动（新功能、界面重构、重大变更）：主版本号 +1，如 `1.2.0 → 2.0.0`
